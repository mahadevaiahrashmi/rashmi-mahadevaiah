...
format_reset_time() {
    ...
    case "$style" in
        time)
            formatted=$(TZ=Asia/Kolkata date -d "@$epoch" +"%H:%M" 2>/dev/null) || \
            formatted=$(TZ=Asia/Kolkata date -j -r "$epoch" +"%H:%M" 2>/dev/null)
            ;;
        datetime)
            formatted=$(TZ=Asia/Kolkata date -d "@$epoch" +"%b %-d, %H:%M" 2>/dev/null) || \
            formatted=$(TZ=Asia/Kolkata date -j -r "$epoch" +"%b %-d, %H:%M" 2>/dev/null)
            ;;
        *)
            formatted=$(TZ=Asia/Kolkata date -d "@$epoch" +"%b %-d" 2>/dev/null) || \
            formatted=$(TZ=Asia/Kolkata date -j -r "$epoch" +"%b %-d" 2>/dev/null)
            ;;
    esac
...
if $effective_builtin; then
    ...
    if [ -n "$builtin_five_hour_pct" ]; then
        ...
        if [ -n "$builtin_five_hour_reset" ] && [ "$builtin_five_hour_reset" != "null" ]; then
            five_hour_reset=$(TZ=Asia/Kolkata date -j -r "$builtin_five_hour_reset" +"%H:%M" 2>/dev/null || TZ=Asia/Kolkata date -d "@$builtin_five_hour_reset" +"%H:%M" 2>/dev/null)
            [ -n "$five_hour_reset" ] && out+=" ${dim}@${five_hour_reset} IST${reset}"
        fi
    fi
    ...
    if [ -n "$builtin_seven_day_pct" ]; then
        ...
        if [ -n "$builtin_seven_day_reset" ] && [ "$builtin_seven_day_reset" != "null" ]; then
            seven_day_reset=$(TZ=Asia/Kolkata date -j -r "$builtin_seven_day_reset" +"%b %-d, %H:%M" 2>/dev/null || TZ=Asia/Kolkata date -d "@$builtin_seven_day_reset" +"%b %-d, %H:%M" 2>/dev/null)
            [ -n "$seven_day_reset" ] && out+=" ${dim}@${seven_day_reset} IST${reset}"
        fi
    fi
